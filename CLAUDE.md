...
- **인스타그램**: @hyuk.xyz
- **현재 상황**: 군복무 중 — 평소에는 **아이폰으로만 작업 가능**. 휴가/외출 때만 PC 작업 가능. **보안상 카메라 사용 불가**. 이 제약을 실행 가능성 판단 시 고려할 것 (촬영/녹화 불가, 파일 크기, 도구 접근성, 작업 흐름 등). **단, 리서치의 범위·깊이·아이디어 인사이트는 환경에 맞춰 제한하지 말 것** — 지금 당장 실행 불가능한 아이디어라도 기록하고 발전시킨다. 제약은 실행의 조건이지 사고의 조건이 아니다.
