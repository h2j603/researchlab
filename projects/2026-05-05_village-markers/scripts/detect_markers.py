...
def geocode_kakao(query: str, api_key: str, *,
                  region_hint: str | None = None) -> tuple[float, float, str] | None:
    """카카오 장소(키워드) 검색 우선 → 주소 검색 fallback.

    query 예: '고양 백마마을', '판교 봇들마을 1단지'
    region_hint 예: '경기도 고양시 일산동구' — 결과 정렬에 도움

    Returns: (lat, lng, matched_place_name) — 어떤 POI가 잡혔는지 추적 가능. 실패 None.
    """
    headers = {"Authorization": f"KakaoAK {api_key}"}

    # 1순위: 키워드(POI) 검색 — 단지·랜드마크·관공서 등 실제 위치
    try:
        resp = requests.get(
        ...
        print(f"  keyword 검색 실패: {e}", file=sys.stderr)
        docs = []

    # region_hint가 있으면 그 지역 결과 우선
    if docs and region_hint:
        prefer = [d for d in docs if region_hint.split()[0] in d.get("address_name", "")]
        if prefer:
            docs = prefer
    ...
    # 2순위: 주소 검색 fallback (region_hint 또는 query 그대로)
    addr_query = region_hint or query
    try:
        resp = requests.get(
            "https://dapi.kakao.com/v2/local/search/address.json",
            params={"query": addr_query},
            headers=headers,
            timeout=10,
        )
        resp.raise_for_status()
        addr_docs = resp.json().get("documents", [])
    ...
    if not addr_docs:
        return None
    d = addr_docs[0]
    return (float(d["y"]), float(d["x"]), d.get("address_name", addr_query))
...
def process_candidate(client: Anthropic, candidate: dict, out_dir: Path,
    ...
    name = candidate["name"]
    cid = candidate["id"]
    # 키워드 검색용: 'search_query' 우선, 없으면 마을 이름으로
    query = candidate.get("search_query") or name
    region = candidate.get("address")  # 결과 좁히기용 힌트
    print(f"\n[{cid}] {name} — query: {query}", file=sys.stderr)

    geo = geocode_kakao(query, kakao_key, region_hint=region)
    if geo is None:
        return {**candidate, "status": "geocode_failed"}
    lat, lng, matched = geo
    print(f"  → {matched} ({lat:.6f}, {lng:.6f})", file=sys.stderr)
