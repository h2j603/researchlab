...
**우리는 표석의 정확한 좌표를 모른다.** 이전 검색 라운드에서 표석 사진·좌표를 얻지 못했고, 가진 정보는 ‘마을 이름 + 동(洞)’ 수준뿐. 그래서 코드는 다음 전략을 쓴다:

1. **카카오 장소(POI) 키워드 검색**(1순위) — “일산 백마마을” → 카카오에 단지 POI로 등록되어 있으면 단지 입구 근처 좌표 반환
2. **주소 검색**(2순위) — POI 미등록 시 동·리 중심 좌표

신도시 ○○마을(G3)은 대부분 POI로 등록되어 있어 잘 잡힌다. 농촌 자연부락(G2)은 등록 안 된 경우가 많아 **동·리 중심 좌표**만 잡히고, 그 좌표에서 4방향을 찍어도 표석이 멀리 있을 수 있다. **즉 hit rate는 G3 > G2** 순이며, G2는 사실상 답사 후보 추출 용도.

이 한계는 자동화의 본질적 한계 — 좌표 데이터셋이 없으므로 “찍어보고 운 좋으면 잡힌다”는 블라인드 프로브 성격을 완전히 벗어날 수 없다. 정확도를 더 높이려면:

- L1 데이터셋 매칭(`match_heritage.py`)으로 좌표 직접 확보 — 매칭률 40~60% 추정
- 실제 답사로 좌표 직접 측정 (휴가 시)
- 카카오 POI 키워드를 ‘{단지명} 1단지 정문’처럼 더 구체화 (실험 필요)

### 그 외 한계

- **Google Street View 커버리지**: 한국 도시 도로는 대체로 잘 잡히지만, 단지 입구가 좁은 골목이거나 사유지 안쪽이면 누락. `marker_present: false`가 모두 ‘없다’를 의미하지 않는다 — ‘로드뷰에 안 잡혔다’일 수 있다.
- **카메라 방향의 한계**: 4방향 캡처로도 표석을 정면에서 못 잡을 수 있다. 표석은 보통 정문 옆 화단에 있어 도로뷰의 측면에 잡힌다.
- **Claude Vision의 한계**: 돌 종류 (S1)/(S2)/(S3) 구분은 사진 해상도·조명에 따라 오차. (S4)/(S5)는 더 어려움. 각자 식별은 정면 클로즈업이 있을 때만 신뢰 가능.
- **자동화의 본질적 함정**: 본 리서치의 핵심이 ‘현장 검증’이라는 점에서, 자동화는 **1차 후보 추출용**으로만 쓴다. §10 변수의 최종 측정은 사람이 직접 확인.
