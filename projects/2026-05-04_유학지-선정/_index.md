---
title: "유학지 선정 — 독일 vs 네덜란드"
date: 2026-05-04
updated: 2026-05-04
status: in-progress
type: research
tags: [career-planning, graphic-design]
related: ["findings.md", "sources.md", "_critique.md"]
confidence: medium
---

# 유학지 선정 — 독일 vs 네덜란드

## 한 줄 요약 (2026-05-04 최종)
**최종 후보 6개 확정**: KABK Type and Media (활자) / KABK Non Linear Narrative (리서치 내러티브) / Sandberg Design (비평) / Werkplaats Typografie (출판) / **DAE Critical Inquiry Lab (학문적 리서치)** / Bauhaus Weimar MFA (매체). 콘텐츠 다각화 + 비용 양극 (Bauhaus·Type and Media = 안전, NLN·Werkplaats = 중간, Sandberg·DAE = 도전). 6개 모두 Bologna MA/MFA, 영어 트랙.

→ **[최종 보고서 (final-report.md)](final-report.md) 참조**

## 전제 조건 (사용자 확정)
- **소속**: 홍익대학교 시각디자인 학사 재학중
- 학비 부담은 불가 → 비EU 국제학생 풀 학비 못 냄 (장학금 또는 무상 학교)
- 후보국: 독일 / 네덜란드 / 스위스 → **스위스 탈락**
- 영어 학습 가능, 독일어 학습 의향 있음
- 장학금 도전 의지 있음, 비용 부담 일부 감수 ("도전해보려고")
- **전역**: 2027-02
- **학사 졸업**: 2029-02 (2028년 2학기 마치고)
- **희망 MA 입학**: 2029-09 (가을학기)

## 지원 학교 컷 (6개 최종 확정)

**메인 도전 트랙 (콘텐츠 우선, 학비·합격 도전)**
- Sandberg Instituut Design MA (비평적 디자인 본진)
- Werkplaats Typografie MA (인디 출판·Karel Martens 학파)
- KABK Non Linear Narrative MA (사회 현상 리서치·내러티브)
- **DAE Critical Inquiry Lab MA (학문적 리서치·분석적 결)**

**메인 안전 트랙 (학비 안전망)**
- KABK Type and Media MA (1년, 활자 글로벌 톱)
- Bauhaus-Universität Weimar MFA Media Art and Design (무상 + 영어 + DAAD)

**드롭됨**: HfG Karlsruhe (Diplom·구조적 비효율), HfG Offenbach·Burg Halle (독일어), UdK Berlin (재검토 가능)

## 문서 구성
- **[final-report.md](final-report.md) — 🎯 최종 통합 보고서 (의사결정 + 비교 + 자격증 + 일정 + 비평)**
- **[graduate-works-links.md](graduate-works-links.md) — 📱 졸업작품 직접 보기 (6개 학교 모바일 친화 링크)**
- **[graduate-works-topics.md](graduate-works-topics.md) — 📊 졸업작품 주제·리서치 방법론 패턴**
- **[competitions.md](competitions.md) — 🏆 유학 전 도전할 해외 공모전 13선**
- **[competitions-strategy.md](competitions-strategy.md) — 🎯 TDC·INTL·Sappi 심층 분석 + 도전 전략**
- [findings.md](findings.md) — 학교별 비교표, 비용 분석, 장학금 옵션
- [school-profiles.md](school-profiles.md) — 9개 프로그램 개별 프로필
- [timeline.md](timeline.md) — 분기별 일정표
- [sources.md](sources.md) — 출처
- [_critique.md](_critique.md) — 비평 (이 결정의 약한 고리)

## 다음 액션 (2026-05 즉시)
- [ ] 매월 사회 현상 리서치 노트 1편 작성 시작 (이 리서치 랩에) — 동기서·포트폴리오 1차 자료
- [ ] 영어 공부 루틴 확정 (IELTS 7.0 목표, 2027-03 1차 응시 예정)
- [ ] Sandberg 졸업생 1~2명 작업 분석 노트 작성 (이 리서치 랩에)
- [ ] 다음 외출 때 관정·일주재단 모집 요강 다운로드
- [ ] 추천 가능한 학부 교수 후보 1~2명 미리 정리

전체 일정은 [timeline.md](timeline.md) 참조.

## 관련 문서
- [전체 인덱스](../../_index.md)
